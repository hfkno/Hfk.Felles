...
namespace Hfk.Felles.Tests.Extensions
{
    ...
    public class Validations
    {
        ...
        private readonly List<string> invalidDUFnrs = new List<string>()
        {
            "12345678901",
            "",
            "1898z3880209",
        ...
        [Test]
        public void can_find_invalid_dufnumbers()
        {
            invalidDUFnrs.ForEach(fhnr =>
                    Assert.That(fhnr.ErGyldigDUFNummer(), Is.False,
                                "{0} skal ikke være gyldig".FormatWith(fhnr))
                             );
        }
        ...
        [Test]
        public void can_handle_bad_values()
        {
        ...
        private static void CheckAllValidations(string empty)
        {
            Assert.That(empty.ErGyldigDUFNummer(), Is.False);
            Assert.That(empty.ErGyldigDNummer(), Is.False);
            Assert.That(empty.ErGyldigFHNummer(), Is.False);
            Assert.That(empty.ErGyldigFNummer(), Is.False);
            Assert.That(empty.ErGyldigFødselsNummer(), Is.False);
            Assert.That(empty.ErGyldigHNummer(), Is.False);
        }
    }
}
